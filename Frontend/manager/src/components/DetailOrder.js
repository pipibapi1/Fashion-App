...
export default function DetailOrder(props) {
    ...
    //         axios.post("https://fashion-app-no-name.herokuapp.com/revenue/searchItem", {searchItem: item}).then((response) => {
    ...
    React.useEffect(() => {     
        if(items.length < props.order.listItemID.length) props.order.listItemID.map((item, index) => {
            const getData = async () => {  
            await axios.post("https://fashion-app-no-name.herokuapp.com/revenue/searchItem", {searchItem: item}).then((response) => {
